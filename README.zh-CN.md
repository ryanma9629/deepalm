...
| --- | --- | --- | --- |
| 1. 在 MacBook 上用小资源跑通全流程 | `configs/corrected-pilot.yaml` | 校准、情景生成、Reference Bank、5/15 年 BM^D/MM 训练、冻结 baseline、评估与报告能完整衔接。 | 收敛、经济效果优越、论文数值复现或银行模型批准。 |
| 2. 尽量贴近论文设定 | `paper_scale` + `paper` 网络；先用公开数据，再迁移银行数据 | 配置层可以表达论文尺度的资源与网络目标。 | 今天就能完成端到端论文尺度复现；公共 runner 目前刻意只开放本机验证流程。 |
...
检查各 run bundle 的 `manifest.json` 和生成的
`corrected-pilot-report.json`。本机验收的标准是：四个成员的证据链完整、身份关联正确、优化更新和评估结果均为有限值；它不是经济验收。目标 M5 MacBook 上的一次实测中，pilot 用时 151.30 秒，随后锁定评估用时 21.04 秒；这只是容量规划观察值，不是性能承诺。

如需执行旧版但覆盖更多策略的 CPU/float64 开发流程，可运行：
...
先将 `configs/quick-skeleton.yaml` 复制到版本控制之外的目录，修改上述字段，以及设备、输出目录、资源上限和新的 run 名称。先执行 `plan`；该命令只解析和展示配置，不会生成情景或开始训练：

```bash
uv run deepalm plan --config path/to/paper-oriented.yaml
```

这里有一个必须明确的当前边界：公共的 `workflow`/`train` runner 只允许有界的 `development-validation` 配置。若把 `paper_scale` 与 `methodologically-reproduced` 配置直接交给它，它会拒绝执行，而不是悄悄带着本机假设运行。这不是命令写错，而是为避免把不完整证据包装成论文复现。要完成真正的论文导向训练，下一项工程工作是公开一个 research runner；之后再在选定的计算环境上从头训练。
...
1. **制作导入型 Reference Bank 快照。** 按照 [Reference Bank 输入合同](docs/reference-bank-inputs.md) 准备：估值日和市场身份、六组 180 月合同梯、固定利率贷款 cohort、四种存款参考期限类别、有日期的初始存款利率历史、产品假设、单位和来源信息。先独立校验快照：
