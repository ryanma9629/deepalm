...
`schema_version` is `3`. Each snapshot also carries a required
...
Schema version 1 lacks fixed-rate loan cohorts and version 2 lacks deposit
reference-term schedules. Both are rejected: neither fixed-rate contracts nor
the original deposit reference-term class can be inferred from aggregate cash
flows. Imported bank data must map contract-level principal schedules, locked
rates, and deposit reference-term schedules explicitly.
