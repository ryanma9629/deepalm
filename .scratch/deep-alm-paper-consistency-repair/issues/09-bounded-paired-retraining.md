...
2026-09-09 — 已建立并运行最小可重复用例
`tests/test_training.py::test_paper_bmd_first_pilot_batch_characterizes_the_flat_loss`。
它在 CPU float64 下以约 1.6 秒重放相同的 Paper BM^D 15 年首 batch（8 条路径、登记市场/
目标随机流）；目标损失、约束损失和所有 BM^D 参数梯度均严格为零，故排除 MPS/RAdam 特有故障。
单变量探针显示，只将贷款计息改为月化时，终值权益均值从 24,612 mCHF 降至 1,785 mCHF 并恢复正损失；
仅切换 PCA 市场口径或取消 treasury 交易会出现约束损失。根因是 ADR 锁定的 Paper 未年化贷款计息使终值
远高于目标，且既有非对称目标函数在“高于目标且无违规”区域梯度为零。该口径选择与本票每个作业须有
参数更新的目标冲突；需要单独决定 Paper 分支是保留失败证据、调整其训练目标，还是调整零梯度处理。
